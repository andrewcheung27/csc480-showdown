...
from showdown.engine.select_best_move import pick_safest, pick_safest_custom
...
def pick_safest_move_using_dynamic_search_depth_custom(battles):
    """
    Dynamically decides how far to look into the game.
    ...
    decision, payoff = pick_safest_custom(all_scores, remove_guaranteed=True)
    bot_choice = decision[0]
    logger.debug("Safest: {}, {}".format(bot_choice, payoff))
    logger.debug("Depth: {}".format(search_depth))
    return bot_choice